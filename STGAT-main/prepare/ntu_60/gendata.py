    ...
    with open(file, 'r') as f:
        ...
        for t in range(skeleton_sequence['numFrame']):
            ...
            for m in range(frame_info['numBody']):
                ...
                #bodyID：身体的唯一标识符。如果在一个场景中有多个人，每个人的bodyID都会不同
                #clipedEdges：表示身体部分是否被视野边缘剪切。例如，如果一个人站在摄像机视野的边缘，他们的一部分身体可能不会出现在视频中，这将通过此标志进行指示
                #handLeftConfidence和handRightConfidence：这些表示左手和右手的追踪置信度。高的置信度意味着系统认为它准确地追踪了手的位置
                #handLeftState和handRightState：这些表示左手和右手的状态，例如是否握拳、是否张开等
                #isRestricted：此标志表示是否存在对身体进行追踪的限制。例如，如果一个人太远，系统可能无法准确地追踪他们的运动，这将通过此标志进行指示
                #leanX和leanY：这些表示身体的倾斜程度。例如，如果一个人向左或向右倾斜，这将通过这些值进行表示
                #trackingState：这表示身体的追踪状态。例如，系统可能在一段时间内丢失对身体的追踪，然后重新找到它。这将通过此状态进行表示
                body_info = {
                    k: float(v)
...
def ske_vis(data, **kwargs):
    from skeleton import vis
    from ntu_skeleton import edge
    vis(data, edge=edge, **kwargs)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NTU-RGB-D Data Converter.')
    parser.add_argument('--data_path', default='E:/MSCdataset/bvh-Nor/skeleton')
    parser.add_argument('--ignored_sample_path',
                        default='D:/Project/PyCharmP/nturgbd_skeletons_s001_to_s017/test.txt')

    parser.add_argument('--out_folder', default='E:/MSCdataset/bvh-Nor/Train_data')
